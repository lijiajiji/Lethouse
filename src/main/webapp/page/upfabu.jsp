...
action="/page/upHouse" enctype="multipart/form-data">
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
  ...
  <TR>
    ...
    <TD><SELECT class=text name=typeId>
      ...
      <option <c:if test="${t.id==house.typeId}">selected="selected"</c:if>
...
name=floorage></TD></TR>
  ...
  <TR>
    ...
    <TD>区：<SELECT class=text id="district" name=district_id>
      <c:forEach items="${districts}" var="d">
        <option <c:if test="${house.distrctid==d.id}"> selected="selected"</c:if>
  ...
  <TR>
    ...
    <TD>
      ...
      <input type="hidden" mame="oldPic" value="${house.path}">
